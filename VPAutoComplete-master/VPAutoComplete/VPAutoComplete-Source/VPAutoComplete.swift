...
class VPAutoComplete: UIView {
    ...
    @objc func didChangeText(textField: UITextField) {
        if var textInField = onTextField.text {
            let textInFieldCount = textInField.count
            if textInFieldCount == 0 || textInField.last == "," {
                is_filter = false
                self.tableView?.reloadData()
                strStartInd = 0
                if textInField.last == "," {
                    strStartInd = textInFieldCount
                    self.isShowView(is_show: true)
                }
            }else{
                if textInFieldCount == strStartInd - 1 {
                    strStartInd = 0
                ...
                textInField = String(textInField.suffix(textInFieldCount - strStartInd))
                is_filter = true
    ...
    func attributedText(withString string: String, boldString: String, font: UIFont) -> NSAttributedString {
        ...
        let boldFontAttribute: [NSAttributedString.Key: Any] = [NSAttributedString.Key.font: font.withSize(font.pointSize)]
